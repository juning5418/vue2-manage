...
<script>
    ...
    export default {
        ...
        methods: {
            async initData(){
                try{
                    const countData = await getIndexTypeCount();
            ...
            async getSelectItemData(row, type){
                const indexType = await getIndexTypeDetail(row.id);
                this.selectTable = {...row, ...{name: indexType.name, image: indexType.image, sort: indexType.sort}};
            ...
            async handleDelete(index, row) {
                try{
                    const res = await deleteIndexType(row.id);
                    if (res.status == 1) {
                        this.$message({
                            type: 'success',
                            message: '删除商品成功'
            ...
            async update(){
                ...
                try{
                    ...
                    const res = await updateIndexType(postData)
                    if (res.status == 1) {
                        this.$message({
                            type: 'success',
                            message: '更新信息成功'
